...
public class CollisionBox {
    ...
    public CollisionBox(Vector offset, Vector center, Vector size, Quaterniond rotation) {
        this(offset, center, size, rotation, (byte) (COPY_YAW | COPY_PITCH));
    }

    public CollisionBox(Vector offset, Vector center, Vector size, Quaterniond rotation, byte copyRotationType) {
        this(offset, center, size, rotation, Color.RED, copyRotationType);
    }

    public CollisionBox(
            Vector offset, Vector center, Vector size, Quaterniond rotation, Color debugColor, byte copyRotationType) {
        this.offset = offset;
        this.center = center;
        this.size = size;
    ...
    public boolean isCollidingSkull(Entity entity) {
        ...
        BoundingBox entityHeadBox =
                entityBoundingBox.clone().expand(0, (entityBoundingBox.getHeight() - eyeHeight * 2) / 2, 0);
    ...
    public List<Pair<Vector, Vector>> getOutline() {
        ...
        Pair<Vector, Vector>[] lines =
                new Pair[] { // ToDo add line class to store data about each line of hitbox outline
                    // x
    ...
    public void updateCollider(Location location) {
        updateCollider(
                new Vector(location.getX(), location.getY(), location.getZ()), location.getPitch(), location.getYaw());
    }
}
